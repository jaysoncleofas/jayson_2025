    ...
    <div class="relative px-4 sm:px-8 lg:px-12">
      <div class="mx-auto max-w-2xl lg:max-w-5xl">
        <header class="max-w-full">
          <h1 class="text-4xl font-bold tracking-tight text-zinc-800 sm:text-5xl dark:text-zinc-100">{{ get_setting('projectsTitle') }}</h1>
          <p class="mt-6 text-base text-zinc-600 dark:text-zinc-400">{{ get_setting('projectsDescription') }}</p>
        </header>
            ...
            @forelse ($projects as $project)
              <li class="group relative flex flex-col items-start">
                <div class="relative z-10 flex h-12 w-12 items-center justify-center rounded-full bg-white shadow-md ring-1 shadow-zinc-800/5 ring-zinc-900/5 dark:border dark:border-zinc-700/50 dark:bg-zinc-800 dark:ring-0">
                  <img alt="" loading="lazy" width="32" height="32" decoding="async" data-nimg="1" class="h-8 w-8 object-contain" src="{{ '/storage/'.$project->photo }}" style="color: transparent;">
                </div>
                ...
                <div class="relative z-10 mt-2 text-sm text-zinc-600 dark:text-zinc-400">{!! str( $project->description)->sanitizeHtml() !!}</div>
                <a
                ...
                >
                  ...
                  <span class="ml-2">{{ $project->site }}</span>
                </a>
              </li>
            @empty
              No Data Available
